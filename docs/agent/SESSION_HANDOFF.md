...
Implement the Curse Slot Machine web game prototype branch by branch from a fresh user-owned clone. Branches through `feature/showcase-step-header-spacing` are merged; `feature/offline-font-fallback` is open as draft PR #22; current work is `feature/ui-adapter-synergy-progress`. Each feature branch should be verified with typecheck, unit tests, and build before a draft PR is opened. Do not merge without explicit user approval.
...
| `feature/ui-adapter-synergy-progress` | pending | pending draft PR | In progress |
...
- `feature/ui-adapter-synergy-progress` is the current branch and is pending draft PR creation.
...
- Remaining issues: open draft PR after commit/push; next structured migration slice should address item-specific UI projection.
