...
- OMX does not own implementation planning or production source-code execution.
- OMX is used only after a playable milestone for UX, level-design, visual-design, and gameplay-QA feedback.
- Do not run Superpowers implementation agents and OMX review agents against the same writable checkout concurrently.
- OMX reviewers do not modify production source code unless the user explicitly starts a separate implementation task.
- Reviewers write evidence-backed findings under `docs/reviews/`.
- Only feedback explicitly accepted by a human may return to the Superpowers implementation workflow.
- Context7 is used only when current library or API documentation is needed.
- Playwright MCP is used for exploratory browser review; committed Playwright tests are the source of repeatable regression checks.
- GitHub is the shared source for review target commits, pull requests, accepted findings, and implementation issues.
- GitHub access is read-only by default. Creating or changing issues, pull requests, comments, labels, branches, or merge state requires explicit human approval.
- Never use `--madmax` or bypass approval and sandbox controls for this project.

## Project scope

- Keep shared configuration in this repository whenever the tool supports project scope.
- Do not commit API keys, OAuth tokens, cookies, browser profiles, or machine-local secrets.
- Do not copy Codex marketplace plugins or user-home configuration into the repository.
- Use `127.0.0.1` for local browser testing unless a task explicitly needs another interface.

## Review artifacts

- Store milestone reviews under `docs/reviews/milestone-XX/`.
- Each finding must include evidence: a screenshot, seed, turn log, console output, or reproduction steps.
- Review-only agents should propose experiments, not edit production code.
