...
import { getOriginDefinition, getOriginTrait, type OriginId } from './OriginCatalog'
...
export class GameEngine {
  ...
  dispatch(command: GameCommand): GameEvent[] {
    ...
    switch (command.type) {
      case 'SELECT_ORIGIN':
        return this.selectOrigin(command.originId)
      case 'START_RUN':
  ...
  private startRun(): GameEvent[] {
    ...
    if (!origin) return this.reject('START_RUN', 'an origin must be selected before starting a run')
    this.state = this.createConfiguredInitialState(this.state.seed, origin)
    this.rng = createSeededRngFromSnapshot(this.state.rng)
  ...
  private selectOrigin(origin: OriginId): GameEvent[] {
    if (this.state.phase !== 'idle') {
      return this.reject('SELECT_ORIGIN', 'an origin can only be selected while idle')
    }
    this.state = {
      ...this.state,
      selectedOrigin: origin,
    }
    return [{ type: 'ORIGIN_SELECTED', originId: origin }]
  }

  private createConfiguredInitialState(seed: RngSeed, origin: OriginId | null = null): GameState {
    const state = createInitialGameState(seed)
    if (origin) {
      const definition = getOriginDefinition(origin)
      state.selectedOrigin = origin
      state.originTraitState = {
        initialBlockPending: definition.initialBlock > 0,
        freeRerollAvailable: false,
      }
      state.economy.gold = definition.gold
      state.combat.player = {
        ...state.combat.player,
        maxHealth: definition.maxHealth,
        health: definition.maxHealth,
        block: definition.initialBlock,
      }
      const reward = MVP_BUILD_CATALOG.rewards.find((candidate) => candidate.id === definition.startingRewardId)
      if (!reward) throw new Error(`Missing origin starting reward: ${definition.startingRewardId}`)
      state.build = applyReward(state.build, { kind: reward.kind, id: reward.id }, MVP_BUILD_CATALOG).build
    }
  ...
  private enterNextStage(): GameEvent[] {
    ...
    const enteringCombat = isCombatStage(stage)
    const preserveInitialBlock = enteringCombat && this.state.originTraitState.initialBlockPending
    this.state = {
      ...this.state,
      phase: getPhaseForStage(stage),
      run,
      combat: enteringCombat ? createStageCombatState(this.state.combat, stage, preserveInitialBlock) : this.state.combat,
      originTraitState: enteringCombat
        ? {
  ...
  private rerollUnlocked(): GameEvent[] {
    ...
    const freeReroll = this.state.selectedOrigin === 'GAMBLER' && this.state.originTraitState.freeRerollAvailable
    const curseCost = freeReroll ? 0 : getCombatRerollCurseCost(this.state.slot.locks)
    ...
    return [
      ...
      ...(freeReroll ? [{
        type: 'ORIGIN_TRAIT_TRIGGERED', originId: 'GAMBLER', effect: 'free_reroll', amount: getCombatRerollCurseCost(this.state.slot.locks),
      } as const] : []),
  ...
  private resolveCombatSlot(
    ...
    const resolution = resolveCombatSlot(this.state.combat, command.result, {
      ...
      ...(this.state.selectedOrigin ? { originTrait: getOriginTrait(this.state.selectedOrigin) } : {}),
      lockedReels,
    })
    const gamblerJackpot = this.state.selectedOrigin === 'GAMBLER' && command.result.modifier === 'x3'
    ...
    this.state = {
      ...
      turn,
      economy: gamblerJackpot
        ? { ...this.state.economy, gold: this.state.economy.gold + 25 }
        : this.state.economy,
      run,
      combat: {
    ...
    const bonusStrike = resolution.events.find((event) => event.type === 'ORIGIN_TRAIT_TRIGGERED')
    if (bonusStrike?.type === 'ORIGIN_TRAIT_TRIGGERED') {
      events.push({ type: 'ORIGIN_TRAIT_TRIGGERED', originId: 'SWORDSMAN', effect: 'bonus_strike', amount: bonusStrike.amount })
    }
    if (gamblerJackpot) {
      events.push({ type: 'ORIGIN_TRAIT_TRIGGERED', originId: 'GAMBLER', effect: 'jackpot', amount: 25 })
    }
    if (this.state.selectedOrigin === 'PRIEST' && (command.result.action === 'shield' || command.result.action === 'heart')) {
      events.push({ type: 'ORIGIN_TRAIT_TRIGGERED', originId: 'PRIEST', effect: 'purify', amount: 1 })
    }
  ...
  private chooseReward(command: Extract<GameCommand, { type: 'CHOOSE_REWARD' }>): GameEvent[] {
    const offered = this.state.rewards.options.find((reward) => reward.id === command.rewardId)
    if (this.state.phase !== 'reward' || !offered) {
      return this.reject('CHOOSE_REWARD', 'reward must be selected from the active offer')
    }
    const reward = { kind: offered.kind, id: offered.id } as const
    const result = applyReward(this.state.build, reward, MVP_BUILD_CATALOG)
...
function createStageCombatState(
  previous: GameState['combat'],
  ...
  return createCombatState({
    player: {
      ...
      block: preserveBlock ? previous.player.block : 0,
    },
    enemy: {
      name: profile.name,
