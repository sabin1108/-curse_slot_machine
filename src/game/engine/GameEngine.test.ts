...
describe('GameEngine', () => {
  ...
  it('carries player health and curse into the next combat while scaling elite encounters', () => {
    ...
    const reward = engine.getState().rewards.options[0]
    engine.dispatch({ type: 'CHOOSE_REWARD', reward: { kind: reward.kind, id: reward.id } })
    engine.dispatch({ type: 'ENTER_NEXT_STAGE' })
    ...
    engine.dispatch({ type: 'CHOOSE_REWARD', reward: { kind: secondReward.kind, id: secondReward.id } })
    engine.dispatch({ type: 'ENTER_NEXT_STAGE' })
    ...
    engine.dispatch({ type: 'CHOOSE_REWARD', reward: { kind: thirdReward.kind, id: thirdReward.id } })
    engine.dispatch({ type: 'ENTER_NEXT_STAGE' })
  ...
  it('can complete the full fifteen-stage normal run without a post-boss reward', () => {
    ...
    while (engine.getState().run.status !== 'victory') {
      ...
      if (state.phase === 'battle') {
        ...
        const reward = engine.getState().rewards.options[0]
        if (reward) engine.dispatch({ type: 'CHOOSE_REWARD', reward: { kind: reward.kind, id: reward.id } })
